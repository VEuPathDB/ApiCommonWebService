...
public class TileMatcher {
    ...
    public static List<MatchWithContext> match(InputStream sequenceInput,
                                               Pattern pattern,
                                               int contextLength) throws Exception {
        boolean first = true;
        ...
        try (final Reader streamReader = new InputStreamReader(sequenceInput);
            ...
            do {
                ...
                if (matcher.find()) {
                    ...
                    matches.add(new MatchWithContext(leadingContext, matcher.group(), trailingContext));
                }
    ...
    private static class SequenceBuffer {
        ...
        public CharBuffer shiftBuffers() {
            if (buffer1 == currentBuffer) {
                buffer2.put(0, buffer1, BUFFER_SIZE + overlapWindow, overlapWindow);
                currentBuffer = buffer2;
            } else {
                buffer1.put(0, buffer2, BUFFER_SIZE + overlapWindow, overlapWindow);
                currentBuffer = buffer1;
            }
