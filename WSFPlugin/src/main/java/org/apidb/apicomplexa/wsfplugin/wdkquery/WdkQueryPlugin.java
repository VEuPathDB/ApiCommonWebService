...
public class WdkQueryPlugin extends AbstractPlugin {
  ...
  public int execute(PluginRequest request, PluginResponse response) throws PluginModelException, PluginUserException {
    ...
    try {
      ...
      // ensure the new guest user_id gets replicated in the component site user database
      // when in a different campus (N/S) from the portal's user database
      // this is only an issue upon restarting a portal:
      // - the initial service call /service/ (which also returns the ontology name where to get the searches),
      // - returns the organims/project mapping;
