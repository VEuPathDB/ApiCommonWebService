...
my $datasetSelection = $ARGV[2];
my $enrichmentType = $ARGV[3];
...
if (lc($useOrthology) eq "yes"){
    $userGeneListQuery = "select distinct ga.orthomcl_name, \'all\' as organism  from webready.GeneAttributes_p  ga, (" . $idSql . ")id where id.gene_source_id = ga.source_id";
    
}else {
    $userGeneListQuery = "select distinct ga.source_id, ga. organism  from webready.GeneAttributes_p  ga, (" . $idSql . ")id where id.gene_source_id = ga.source_id";
...
WHERE ga.fdiff_abs >=10  and id.gene_source_id = ga.source_id";
...
sub runRUnderRepresented{
    my ($t11,$t21,$t12,$t22) = @_;
    
    my $rCode = <<"RCODE";
    Test <-matrix(c($t11,$t21,$t12,$t22), nrow = 2, dimnames = list(DS = c("DS", "Non-DS"),UL = c("UL", "Non-UL")))
    fisher.test(Test, alternative = "less")\$p
RCODE
...
sub runRBoth{
    ...
    my $rCode = <<"RCODE";
    Test <-matrix(c($t11,$t21,$t12,$t22), nrow = 2, dimnames = list(DS = c("DS", "Non-DS"),UL = c("UL", "Non-UL")))
    fisher.test(Test, alternative = "two.sided")\$p
RCODE
